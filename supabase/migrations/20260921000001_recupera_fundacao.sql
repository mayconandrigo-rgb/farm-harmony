-- Recuperação da fundação agrícola Fazenda 3V (idempotente)
...
drop policy if exists "autenticados gerenciam fazendas" on public.fazendas;
create policy "autenticados gerenciam fazendas" on public.fazendas
  for all to authenticated using (true) with check (true);
drop policy if exists "autenticados gerenciam talhoes" on public.talhoes;
create policy "autenticados gerenciam talhoes" on public.talhoes
  for all to authenticated using (true) with check (true);
drop policy if exists "autenticados gerenciam culturas" on public.culturas;
create policy "autenticados gerenciam culturas" on public.culturas
  for all to authenticated using (true) with check (true);
drop policy if exists "autenticados gerenciam variedades" on public.variedades;
create policy "autenticados gerenciam variedades" on public.variedades
  for all to authenticated using (true) with check (true);
drop policy if exists "autenticados gerenciam safras" on public.safras;
create policy "autenticados gerenciam safras" on public.safras
  for all to authenticated using (true) with check (true);
drop policy if exists "autenticados gerenciam atividades" on public.atividades;
create policy "autenticados gerenciam atividades" on public.atividades
  for all to authenticated using (true) with check (true);

-- Culturas iniciais da 3V (ignora se já existirem)
insert into public.culturas (nome) values ('Soja'), ('Milho'), ('Algodão')
